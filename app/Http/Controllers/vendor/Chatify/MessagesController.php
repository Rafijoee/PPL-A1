    ...
    public function send(Request $request)
    {
        ...
        foreach ($penyuluh as $user2) {
            ...
            if ($kecamatanId === $kecamatanPenyuluh) {
                $id_penyuluh[] = $user2->id;
            }
        }
        $petani = User::where('roles_id', 2)->get();
        $id_petani = [];
        foreach ($petani as $user3) {
            $kecamatanPetani = $user3->profile->kecamatan_id;

            if ($kecamatanId === $kecamatanPetani) {
                $id_petani[] = $user3->id;
            }
        }
        ...
        if ($id == 2){
            return redirect ('/chatify/' . $id_penyuluh[0]);
        }

        if ($id == 3){
            return redirect('/chatify/' . $id_petani[0]);
        }
        // send the response
        return Response::json([
            'status' => '200',
